...
import 'package:movegui/widgets/address/default_store_or_position_address_widget.dart';
...
class DisplayAdressesWidgetState extends ConsumerState<DisplayAdressesWidget> {
 ...
 // AdressModel? currentAddress = null;
  ...
  Future<void> _cancelFct(BuildContext dialogContext, ButtonItem item) async {
    if (!item.enabled) {
  ...
  Future<void> _addAdress(BuildContext dialogContext, ButtonItem item) async {
            ...
            Expanded(
              child:
                  formControllers.length < AppConstants.MAX_ADRESSES
                      ? ButtonWidget(
                        onPressed: (dialogcontext, item) async {
                          _addAdress(dialogcontext, item);
                        },
